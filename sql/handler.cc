  ...
  */
  ...
  if (hton->state == SHOW_OPTION_YES && hton->recover)
  {
    while ((got= hton->recover(hton, info->list, info->len,
                               info->binlog_file, info->binlog_pos,
                               info->binlog_max_gtid)) > 0)
    {
      ...
      for (int i=0; i < got; i ++)
      {
            ...
            tc_heuristic_recover == TC_HEURISTIC_RECOVER_COMMIT)
        {
          ...
          if (xid_to_gtid->gtid.greater_than(recovered_max_gtid) ||
              recovered_max_gtid.sidno != xid_to_gtid->gtid.sidno)
          {
            recovered_max_gtid= xid_to_gtid->gtid;
          }
                        ...
                        const THD *thd, const char *org_table_name)
{
  ...
  if (opt_improved_dup_key_error)
  {
    ...
    const char *query_str = thd->query();
...
int
handler::ha_discard_or_import_tablespace(uint discard)
{
  DBUG_ASSERT(table_share->tmp_table != NO_TMP_TABLE ||
              m_lock_type == F_WRLCK);
  mark_trx_read_write();
